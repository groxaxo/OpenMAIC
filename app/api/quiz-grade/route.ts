...
{"score": <0到${points}的整数>, "comment": "<一两句评语>"}`
      : isEs
        ? `Eres un evaluador educativo profesional. Califica la respuesta del estudiante y proporciona un comentario breve.
Debes responder únicamente en el siguiente formato JSON (sin contenido adicional):
{"score": <entero entre 0 y ${points}>, "comment": "<una o dos frases de comentario>"}`
...
${commentPrompt ? `评分要点：${commentPrompt}\n` : ''}学生答案：${userAnswer}`
      : isEs
        ? `Pregunta: ${question}
Puntuación máxima: ${points} puntos
${commentPrompt ? `Criterios de evaluación: ${commentPrompt}\n` : ''}Respuesta del estudiante: ${userAnswer}`
        : `Question: ${question}
    ...
    try {
      ...
      gradeResult = {
        ...
        comment: isZh
          ? '已作答，请参考标准答案。'
          : isEs
            ? 'Respuesta recibida. Consulta la respuesta modelo.'
