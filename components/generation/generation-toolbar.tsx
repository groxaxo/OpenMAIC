...
import type { CourseLanguage } from '@/lib/types/generation';

// ─── Constants ───────────────────────────────────────────────
const MAX_PDF_SIZE_MB = 50;
const MAX_PDF_SIZE_BYTES = MAX_PDF_SIZE_MB * 1024 * 1024;
const LANGUAGE_CYCLE: CourseLanguage[] = ['zh-CN', 'en-US', 'es-ES'];
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            onClick={() =>
              onLanguageChange(
                LANGUAGE_CYCLE[(LANGUAGE_CYCLE.indexOf(language) + 1) % LANGUAGE_CYCLE.length],
              )
            }
            className={pillMuted}
          >
