...
1. Read `accessCode` from skill config (`~/.openclaw/openclaw.json` or `~/.openjaw/openjaw.json` → `skills.entries.openmaic.config.accessCode`).
...
3. If not found, tell the user to add their access code to the config file:
   ```
   Edit ~/.openclaw/openclaw.json (or ~/.openjaw/openjaw.json) and set skills.entries.openmaic.config.accessCode to your access code (starts with sk-).
   ```
